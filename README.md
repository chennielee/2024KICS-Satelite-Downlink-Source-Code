# 🛰️ Energy-Efficient Ground Station Selection for LEO Downlink  
...
Evaluation uses **pure exploitation** for DQN (**ε=0**), run over **N evaluation episodes** with identical environment settings.
...
- **High-level**: selects an operating mode (e.g., “prioritize energy” vs “prioritize freshness”)
...
├── eval_only.py           # Eval-only rollouts + metrics export (CSV/PNG)
├── random_policy.py       # Visibility-aware random baseline
├── greedy_policy.py       # Closest visible station baseline
...
* **Statistical Significance:** Recommended to run **≥30 eval episodes** to report stable mean/std metrics (10 episodes for quick sanity checks).
